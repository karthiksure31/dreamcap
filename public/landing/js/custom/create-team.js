$(function () {
    ...
    $("#generateTeam").on("click", function () {
        ...
        if (totalPlayer < 12) {
            toastr.warning('You must select at least 12 players');
            return false;
        } else {
            if (countPlayers > 16) {
                toastr.warning('Our recommendation is to limit the number of players to 16');
                return false;
            } else {
                if (countMinMaxTeam1 >= 4 && countMinMaxTeam2 >= 4) {
                    if ((countWk > 0) && (countBat >= 3) && (countAll > 0) && (countBowl >= 3)) {
                        ...
                        toastr.warning('Select a minimum of 1 wicketkeeper, 3 batsmen, 1 all-rounder, and 3 bowlers');
                        return false;
                    }
